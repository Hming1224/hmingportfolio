...
const componentSeeds: ComponentSeed[] = [
  ...
  { slug: "case-before-after", title: "CaseBeforeAfter", titleZh: "前後比較", category: "Case Study", source: "components/case-study/CaseBeforeAfter.tsx", demo: "case-before-after", states: ["desktop horizontal", "mobile stacked"], usage: ["Source-level reusable component for simple two-panel before / after comparisons.", "Current route adoption: no direct usage in Advantech, Crypto Arsenal, or Laushu.", "Use it when the comparison only needs two labeled slots and a connector, without case-study narrative copy.", "Boundary: not a BeforeAfterNarrativeFrame replacement and not a live route pattern yet.", "The .cs-before-after-panel selector boundary belongs to CaseBeforeAfter."], usageZh: ["適合簡單兩欄 before / after 比較的 source-level reusable component。", "目前正式案例頁使用狀態：Advantech、Crypto Arsenal、Laushu 沒有直接使用。", "當比較只需要兩個 labeled slots 與 connector、不需要案例敘事文字時使用。", "邊界：不是 BeforeAfterNarrativeFrame 的替代元件，也還不是 live route pattern。", "`.cs-before-after-panel` selector 邊界屬於 CaseBeforeAfter。"], tokens: ["--cs-surface", "--cs-line", "--cs-accent", "--hm-space-md"] },
  { slug: "before-after-narrative-frame", title: "BeforeAfterNarrativeFrame", titleZh: "敘事型前後比較框架", category: "Case Study", source: "components/case-study/BeforeAfterNarrativeFrame.tsx", demo: "before-after-narrative", states: ["blue", "cyan", "purple", "neutral", "slot-based"], usage: ["Use when a case-study section needs a narrative before / after frame, not just a primitive two-panel comparison.", "Live usage: Advantech SolutionSection Board 1, Crypto Arsenal IterationSection, and Laushu iteration board.", "Frame anatomy: CaseCard shell, scenario badge, title, narrative points, before panel, after panel, comparison layout, and connector.", "BeforeAfterPanel is rendered inside this frame as an internal visual shell; it is not a standalone route-level pattern.", "The frame owns reusable narrative shell and panel structure; the route owns screenshots, redlines, sizing, and project-specific design judgment.", "Advantech Board 2 / 3 remain route-local until the same multi-comparison structure repeats across cases."], usageZh: ["當案例段落需要敘事型 before / after frame，而不只是簡單兩欄比較時使用。", "真實使用位置：Advantech SolutionSection Board 1、Crypto Arsenal IterationSection、Laushu iteration board。", "Frame anatomy：CaseCard 外框、scenario badge、title、narrative points、before panel、after panel、comparison layout 與 connector。", "BeforeAfterPanel 在這個 frame 內作為 internal visual shell 被 render；它不是 standalone route-level pattern。", "Frame 負責可重複的 narrative shell 與 panel structure；route 負責 screenshots、redlines、sizing 與專案特定設計判斷。", "Advantech Board 2 / 3 在相同 multi-comparison 結構跨案例重複前，維持 route-local。"], tokens: ["--cs-accent", "--cs-surface", "--cs-line", "--hm-space-md", "--cs-before-after-state-panel-*"] },
