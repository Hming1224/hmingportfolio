...
const foundations: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    usageZh: [
      ...
      "英文單字需保持可讀，不用逐字強制斷行。",
      "Section heading 目前作為 shared content hierarchy convention 管理，而不是獨立元件。在抽出共用 component API 前，公開頁面應維持一致的 eyebrow、標題與輔助文字節奏。",
    ],
...
const componentSeeds: ComponentSeed[] = [
  ...
  },
  {
    ...
    hideSourceInHeader: true,
    status: "Live component",
    statusZh: "Live component",
    description: "Reveals and hides supporting content within a section.",
    descriptionZh: "用於在同一區塊內展開或收合輔助內容。",
    exampleLabel: "Design System category navigation",
    exampleLabelZh: "Design System 分類導覽",
    demo: "accordion",
    usage: [
      "Use when supporting content should be available but not always visible.",
      "Use for compact FAQs, grouped explanations, or secondary details.",
      "Use for grouped navigation only when the navigation model has clear section hierarchy.",
      "Do not use when all content is critical and should remain visible.",
      "Do not use it as tabs, pagination, or global route navigation.",
    ],
    usageZh: [
      "當輔助內容需要可取得、但不必一直顯示時使用。",
      "適合 compact FAQ、分組說明或次要細節。",
      "只有當導覽模型有清楚 section hierarchy 時，才用於分組導覽。",
      "如果所有內容都是關鍵資訊並應該持續可見，不要使用。",
      "不要把它當成 tabs、pagination 或全站 route navigation。",
    ],
    stateRows: [
      { state: "Collapsed", appliesTo: "AccordionItem / AccordionPanel", trigger: "Item value is not included in the current open values.", behavior: "Panel is hidden while the trigger remains reachable.", whatChanges: "`data-state=\"closed\"`, `aria-expanded=\"false\"`, and `hidden` on the panel.", liveUsage: "Closed categories in Design System navigation." },
      { state: "Expanded", appliesTo: "AccordionItem / AccordionPanel", trigger: "User activates the trigger or the item matches `defaultValue` / `value`.", behavior: "Panel becomes visible and the chevron rotates through CSS.", whatChanges: "`data-state=\"open\"`, `aria-expanded=\"true\"`, and visible region content.", liveUsage: "Current or opened categories in Design System navigation." },
      { state: "Single mode", appliesTo: "Accordion", trigger: "`type=\"single\"`.", behavior: "Opening an item closes the previously open item.", whatChanges: "`onValueChange` receives a string value.", liveUsage: "Component example and Explorer category switcher." },
      { state: "Multiple mode", appliesTo: "Accordion", trigger: "`type=\"multiple\"`.", behavior: "Multiple items can remain open at the same time.", whatChanges: "`onValueChange` receives an array of values.", liveUsage: "DesignSystemDocsNav sidebar and mobile menu." },
      { state: "Trigger interaction", appliesTo: "AccordionHeader", trigger: "Click, Enter, or Space on the header button.", behavior: "The item toggles through the real button element.", whatChanges: "`toggleValue` updates open values.", liveUsage: "All current docs navigation accordions." },
      { state: "Keyboard focus movement", appliesTo: "AccordionHeader", trigger: "ArrowDown / ArrowRight or ArrowUp / ArrowLeft.", behavior: "Focus moves between accordion trigger buttons.", whatChanges: "`focusTrigger` locates `[data-accordion-trigger]` buttons and focuses the next trigger.", liveUsage: "Source-level keyboard affordance." },
    ],
    stateRowsZh: [
      { state: "Collapsed", appliesTo: "AccordionItem / AccordionPanel", trigger: "item value 不在目前 open values 中。", behavior: "panel 隱藏，但 trigger 仍可操作。", whatChanges: "`data-state=\"closed\"`、`aria-expanded=\"false\"`，panel 帶有 `hidden`。", liveUsage: "Design System navigation 中未展開的分類。" },
      { state: "Expanded", appliesTo: "AccordionItem / AccordionPanel", trigger: "使用者啟動 trigger，或 item 符合 `defaultValue` / `value`。", behavior: "panel 顯示，chevron 透過 CSS 旋轉。", whatChanges: "`data-state=\"open\"`、`aria-expanded=\"true\"`，region 內容可見。", liveUsage: "Design System navigation 中目前或已展開的分類。" },
      { state: "Single mode", appliesTo: "Accordion", trigger: "`type=\"single\"`。", behavior: "開啟某個 item 時會收合前一個 item。", whatChanges: "`onValueChange` 回傳 string value。", liveUsage: "元件範例與 Explorer 分類切換。" },
      { state: "Multiple mode", appliesTo: "Accordion", trigger: "`type=\"multiple\"`。", behavior: "多個 item 可以同時保持展開。", whatChanges: "`onValueChange` 回傳 values array。", liveUsage: "DesignSystemDocsNav sidebar 與 mobile menu。" },
      { state: "Trigger interaction", appliesTo: "AccordionHeader", trigger: "點擊、Enter 或 Space 觸發 header button。", behavior: "item 透過真實 button element 切換。", whatChanges: "`toggleValue` 更新 open values。", liveUsage: "目前所有 docs navigation accordions。" },
      { state: "Keyboard focus movement", appliesTo: "AccordionHeader", trigger: "ArrowDown / ArrowRight 或 ArrowUp / ArrowLeft。", behavior: "focus 會在 accordion trigger buttons 之間移動。", whatChanges: "`focusTrigger` 找到 `[data-accordion-trigger]` buttons，並 focus 下一個 trigger。", liveUsage: "source-level keyboard affordance。" },
    ],
    anatomyParts: [
      { part: "Root", description: "Context provider and root wrapper that stores open values and trigger focus management.", owner: "Accordion", code: ".hm-accordion" },
      { part: "Item", description: "Value-bound disclosure row with open / closed data state.", owner: "AccordionItem", code: ".hm-accordion-item[data-state]" },
      { part: "Header / trigger", description: "Button that toggles the item and exposes expanded state.", owner: "AccordionHeader", code: ".hm-accordion-trigger / aria-expanded" },
      { part: "Indicator", description: "Chevron icon that rotates when the item opens.", owner: "AccordionHeader / CSS", code: ".hm-accordion-icon" },
      { part: "Panel", description: "Region connected to the trigger through ids and hidden when collapsed.", owner: "AccordionPanel", code: ".hm-accordion-panel / role=\"region\"" },
    ],
    anatomyPartsZh: [
      { part: "Root", description: "提供 context 的 root wrapper，管理 open values 與 trigger focus。", owner: "Accordion", code: ".hm-accordion" },
      { part: "Item", description: "綁定 value 的 disclosure row，帶有 open / closed data state。", owner: "AccordionItem", code: ".hm-accordion-item[data-state]" },
      { part: "Header / trigger", description: "切換 item 的 button，並暴露 expanded state。", owner: "AccordionHeader", code: ".hm-accordion-trigger / aria-expanded" },
      { part: "Indicator", description: "item 展開時旋轉的 chevron icon。", owner: "AccordionHeader / CSS", code: ".hm-accordion-icon" },
      { part: "Panel", description: "透過 ids 與 trigger 關聯的 region，收合時 hidden。", owner: "AccordionPanel", code: ".hm-accordion-panel / role=\"region\"" },
    ],
    codeGuidance: {
      importPath: "components/ui/Accordion.tsx",
      example: "import { Accordion, AccordionHeader, AccordionItem, AccordionPanel } from \"@/components/ui/Accordion\";\n\n<Accordion defaultValue=\"overview\" type=\"single\">\n  <AccordionItem value=\"overview\">\n    <AccordionHeader>Overview</AccordionHeader>\n    <AccordionPanel>\n      <p>Supporting content.</p>\n    </AccordionPanel>\n  </AccordionItem>\n</Accordion>",
      props: [
        { name: "type", type: "\"single\" | \"multiple\"", description: "Controls whether one or multiple items can stay open." },
        { name: "defaultValue", type: "string | string[]", description: "Initial open value for uncontrolled accordions." },
        { name: "value", type: "string | string[]", description: "Controlled open value." },
        { name: "onValueChange", type: "(value: string | string[]) => void", description: "Receives the next open value when a trigger toggles." },
        { name: "AccordionItem.value", type: "string", description: "Stable item id used by open state and trigger focus movement." },
        { name: "children", type: "ReactNode", description: "Compose items with AccordionHeader and AccordionPanel." },
      ],
      notes: [
        "Use the exported parts together; AccordionItem must be nested inside Accordion.",
        "The component does not expose a disabled item prop today.",
        "Use Tabs when switching primary peer views rather than revealing supporting details.",
      ],
    },
    codeGuidanceZh: {
      importPath: "components/ui/Accordion.tsx",
      example: "import { Accordion, AccordionHeader, AccordionItem, AccordionPanel } from \"@/components/ui/Accordion\";\n\n<Accordion defaultValue=\"overview\" type=\"single\">\n  <AccordionItem value=\"overview\">\n    <AccordionHeader>Overview</AccordionHeader>\n    <AccordionPanel>\n      <p>Supporting content.</p>\n    </AccordionPanel>\n  </AccordionItem>\n</Accordion>",
      props: [
        { name: "type", type: "\"single\" | \"multiple\"", description: "控制一次只能展開一個 item，或可同時展開多個 item。" },
        { name: "defaultValue", type: "string | string[]", description: "uncontrolled accordion 的初始 open value。" },
        { name: "value", type: "string | string[]", description: "controlled open value。" },
        { name: "onValueChange", type: "(value: string | string[]) => void", description: "trigger 切換後回傳下一個 open value。" },
        { name: "AccordionItem.value", type: "string", description: "穩定 item id，用於 open state 與 trigger focus movement。" },
        { name: "children", type: "ReactNode", description: "用 AccordionHeader 與 AccordionPanel 組合 items。" },
      ],
      notes: [
        "請一起使用 exported parts；AccordionItem 必須放在 Accordion 裡。",
        "目前元件沒有 disabled item prop。",
        "若是切換同層級主要視圖，請使用 Tabs，而不是 Accordion。",
      ],
    },
    tokenMappings: [
      { token: ".hm-accordion", role: "Root stack", usage: "Controls vertical item layout." },
      { token: ".hm-accordion-item", role: "Item divider", usage: "Uses `--hm-line` for item separation." },
      { token: ".hm-accordion-trigger", role: "Trigger typography and hit area", usage: "Uses `--hm-space-xs`, `--hm-fs-xs`, and `--hm-fw-bold`." },
      { token: ".hm-accordion-icon", role: "Indicator motion", usage: "Uses `--hm-duration-fast` and `--hm-ease-out` for rotation." },
      { token: ".hm-accordion-panel", role: "Panel rhythm", usage: "Uses compact padding and `--text-secondary` copy color." },
    ],
    tokenMappingsZh: [
      { token: ".hm-accordion", role: "Root stack", usage: "控制垂直 item layout。" },
      { token: ".hm-accordion-item", role: "Item divider", usage: "用 `--hm-line` 分隔 items。" },
      { token: ".hm-accordion-trigger", role: "Trigger typography and hit area", usage: "使用 `--hm-space-xs`、`--hm-fs-xs` 與 `--hm-fw-bold`。" },
      { token: ".hm-accordion-icon", role: "Indicator motion", usage: "用 `--hm-duration-fast` 與 `--hm-ease-out` 控制旋轉。" },
      { token: ".hm-accordion-panel", role: "Panel rhythm", usage: "使用 compact padding 與 `--text-secondary` 內文色。" },
    ],
    tokens: [".hm-accordion", ".hm-accordion-item", ".hm-accordion-trigger", ".hm-accordion-icon", ".hm-accordion-panel", "--hm-line", "--hm-duration-fast", "--hm-ease-out"],
    accessibility: [
      "AccordionHeader renders a real button and exposes `aria-expanded`.",
      "AccordionPanel links back to the trigger with `aria-labelledby` and uses `role=\"region\"`.",
      "Collapsed panels use the native `hidden` attribute.",
      "Arrow keys move focus between triggers; Enter and Space work through native button behavior.",
      "Do not claim disabled item behavior unless a disabled API is added.",
    ],
    accessibilityZh: [
      "AccordionHeader render 真實 button，並暴露 `aria-expanded`。",
      "AccordionPanel 透過 `aria-labelledby` 關聯 trigger，並使用 `role=\"region\"`。",
      "收合的 panel 使用 native `hidden` attribute。",
      "方向鍵會在 triggers 之間移動 focus；Enter 與 Space 由 native button behavior 支援。",
      "除非未來加入 disabled API，否則不要宣稱支援 disabled item behavior。",
    ],
    referenceCards: [
      { label: "Source path", value: "components/ui/Accordion.tsx" },
      { label: "Live usage", value: "Design System Explorer category navigation and DesignSystemDocsNav sidebar / mobile menu" },
      { label: "Status", value: "Live component used by the documentation route" },
      { label: "Boundary", value: "Disclosure component, not tabs, pagination, or global navigation" },
      { label: "Related", value: "Tabs for primary peer view switching" },
    ],
    referenceCardsZh: [
      { label: "Source path", value: "components/ui/Accordion.tsx" },
      { label: "Live usage", value: "Design System Explorer 分類導覽，以及 DesignSystemDocsNav sidebar / mobile menu" },
      { label: "Status", value: "文件 route 正式使用的 live component" },
      { label: "Boundary", value: "Disclosure component，不是 tabs、pagination 或 global navigation" },
      { label: "Related", value: "主要同層級視圖切換請使用 Tabs" },
    ],
    references: ["components/ui/Accordion.tsx", "components/design-system/DesignSystemExplorer.tsx", "components/design-system/DesignSystemDocsNav.tsx", "styles/tokens.css"],
  },
  {
    slug: "floating-input",
    title: "FloatingInput",
  ...
  },
  {
    ...
    descriptionZh: "用於包覆案例頁內容區塊，維持段落 spacing、背景與閱讀節奏一致。",
    ...
    usageZh: [
      ...
      "當 section spacing、背景與閱讀節奏需要對齊案例頁 template 時使用。",
      "不要直接用於首頁或 marketing section，除非未來擴充 component contract。",
    ...
    behaviorZh: [
      ...
      { label: "Children flow", description: "內文、cards、media 或 route-owned boards 會放在 heading 後方的 children slot。" },
      { label: "Class hook", description: "`className` 讓 route 加上 section-specific layout hook，不改 shared component。" },
    ],
    anatomyParts: [
    ...
    anatomyPartsZh: [
      ...
      { part: "Children slot", description: "heading 後方由 route 提供的內容。", owner: "Route / caller", code: "children" },
      { part: "Route class hook", description: "為 layout 需求保留的 optional section-specific class。", owner: "Route / caller", code: "className" },
    ],
    codeGuidance: {
    ...
    codeGuidanceZh: {
      ...
      props: [
        ...
        { name: "children", type: "ReactNode", description: "由 route 提供的 section body content。" },
      ],
      notes: ["route-specific diagram、comparison board 與密集敘事 layout 應放在 children slot。", "若 custom full-bleed story section 不適合這個 wrapper，保留 route-local section。"],
    },
    tokenMappings: [
    ...
    tokenMappingsZh: [
      ...
      { token: ".cs-section-surface", role: "Surface section root", usage: "用於交錯閱讀節奏的共用 surface 背景。" },
    ...
    referenceCardsZh: [
      ...
      { label: "Live usage", value: "Advantech OverviewSection / AnalysisSection / ScenarioSection / SolutionSection" },
      { label: "CSS", value: "styles/case-study.css: .cs-section / .cs-section-surface" },
      { label: "Boundary", value: "案例頁 section wrapper，不是 generic app section" },
    ],
  ...
  },
  {
    ...
    usageZh: [
      ...
      "當媒體需要共用 figure、frame、optional caption 或 scroll shell 時使用。",
    ...
    behaviorZh: [
      ...
      { label: "Media frame", description: "children 會包在 `.cs-media-frame`；裁切與比例由 route content 或 contentClassName 負責。" },
      ...
      { label: "Route ownership", description: "圖片 source、alt text、裁切、object-fit 與 story-specific geometry 由 route 負責。" },
    ],
    anatomyParts: [
    ...
    anatomyPartsZh: [
      ...
      { part: "Media child", description: "由 route 提供的圖片、table、video 或 nested component。", owner: "Route / caller", code: "children" },
    ...
    codeGuidanceZh: {
      ...
      props: [
        ...
        { name: "caption", type: "ReactNode", description: "optional figure caption。" },
        { name: "variant", type: "\"contained\" | \"full\" | \"zoomable\" | \"scroll\"", description: "共用 media shell preset。" },
        { name: "className", type: "string", description: "root 上的 optional route spacing / context class hook。" },
        { name: "contentClassName", type: "string", description: "`.cs-media-frame` 上的 optional route-owned crop / sizing class hook。" },
      ],
    ...
    tokenMappingsZh: [
      { token: ".cs-media", role: "Figure root", usage: "負責案例頁 media shell。" },
    ...
    referenceCardsZh: [
      ...
      { label: "Live usage", value: "Crypto Arsenal CurrentSection / FinalSection / ResearchSection；Advantech AnalysisSection；Laushu media sections" },
      { label: "CSS", value: "styles/case-study.css: .cs-media / .cs-media-frame / .cs-media-caption" },
      { label: "Boundary", value: "案例頁 media frame，不是 generic image component" },
      { label: "Related", value: "當 media 本身有互動時使用 ZoomableImage" },
    ],
  ...
  { slug: "before-after-narrative-frame", title: "BeforeAfterNarrativeFrame", titleZh: "敘事型前後比較框架", category: "Case Study", source: "components/case-study/BeforeAfterNarrativeFrame.tsx", hideSourceInHeader: true, status: "Live route pattern", statusZh: "Live route pattern", description: "A case-study narrative frame for guided before / after comparisons.", descriptionZh: "用於案例頁引導式 before / after 比較的敘事框架。", exampleLabel: "Advantech / SolutionSection / Board 1", exampleLabelZh: "Advantech / SolutionSection / Board 1", demo: "before-after-narrative", usage: ["Use when a case-study section needs a guided before / after narrative.", "Use when the frame must carry scenario, design decision, and comparison structure.", "Live usage: Advantech, Crypto Arsenal, and Laushu.", "The frame owns the repeatable narrative shell; each route owns screenshots, redlines, sizing, and specific design judgment."], usageZh: ["適合用在案例頁需要引導式 before / after 敘事的段落。", "當畫面需要承載情境、設計決策與比較結構時使用。", "已用在 Advantech、Crypto Arsenal、Laushu。", "frame 負責可重複的敘事外框；截圖、redline、尺寸與專案判斷仍由各案例頁負責。"], behavior: [{ label: "Pattern", description: "Live route pattern used by Advantech, Crypto Arsenal, and Laushu." }, { label: "Route-owned content", description: "Routes own screenshots, redlines, sizing, and project-specific judgment." }, { label: "Responsive layout", description: "The comparison stacks on narrow viewports through the production case-study CSS." }, { label: "Panel slots", description: "Before and after content remains route-provided." }, { label: "Comparison flow", description: "The connector communicates relationship, but it cannot be the only meaning." }], behaviorZh: [{ label: "Pattern", description: "Advantech、Crypto Arsenal、Laushu 正式採用的 live route pattern。" }, { label: "Route-owned content", description: "截圖、redline、尺寸與專案判斷由各 route 負責。" }, { label: "Responsive layout", description: "窄版會透過 production case-study CSS 改為上下堆疊。" }, { label: "Panel slots", description: "Before 與 after 內容仍由 route 提供。" }, { label: "Comparison flow", description: "connector 用來傳達關係，但不能成為唯一語意。" }], anatomyParts: [{ part: "Frame shell", description: "Case-study narrative container for a guided comparison.", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative" }, { part: "Scenario badge / header", description: "Identifies the comparison moment and frames the section title.", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative-header / badge" }, { part: "Before panel", description: "Before state rendered through the internal panel shell.", owner: "BeforeAfterNarrativeFrame + BeforeAfterPanel", code: ".cs-before-after-state-panel" }, { part: "After panel", description: "After state rendered through the internal panel shell.", owner: "BeforeAfterNarrativeFrame + BeforeAfterPanel", code: ".cs-before-after-state-panel" }, { part: "Connector", description: "Visual relationship marker between the before and after panels.", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative-connector" }, { part: "Narrative copy", description: "Optional point list supplied by the route when the comparison needs explanation.", owner: "Route / caller", code: "points[]" }, { part: "BeforeAfterPanel internal visual shell", description: "Rendered inside BeforeAfterNarrativeFrame; internal anatomy, not a standalone route-level pattern.", owner: "BeforeAfterPanel", code: "components/case-study/BeforeAfterPanel.tsx" }], anatomyPartsZh: [{ part: "Frame shell", description: "案例頁引導式比較的敘事容器。", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative" }, { part: "Scenario badge / header", description: "標示比較情境，並建立 section title。", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative-header / badge" }, { part: "Before panel", description: "透過 internal panel shell 渲染 before 狀態。", owner: "BeforeAfterNarrativeFrame + BeforeAfterPanel", code: ".cs-before-after-state-panel" }, { part: "After panel", description: "透過 internal panel shell 渲染 after 狀態。", owner: "BeforeAfterNarrativeFrame + BeforeAfterPanel", code: ".cs-before-after-state-panel" }, { part: "Connector", description: "before 與 after panel 之間的視覺關係標記。", owner: "BeforeAfterNarrativeFrame", code: ".cs-before-after-narrative-connector" }, { part: "Narrative copy", description: "需要補充比較說明時，由 route 提供 optional point list。", owner: "Route / caller", code: "points[]" }, { part: "BeforeAfterPanel internal visual shell", description: "在 BeforeAfterNarrativeFrame 內部渲染；它是 internal anatomy，不是獨立 route-level pattern。", owner: "BeforeAfterPanel", code: "components/case-study/BeforeAfterPanel.tsx" }], codeGuidance: { importPath: "components/case-study/BeforeAfterNarrativeFrame.tsx", example: "import { BeforeAfterNarrativeFrame } from \"@/components/case-study/BeforeAfterNarrativeFrame\";\n\n<BeforeAfterNarrativeFrame\n  badge=\"Scenario 1\"\n  title=\"AI Chatbot component\"\n  beforeLabel=\"Before\"\n  afterLabel=\"After\"\n  before={<BeforeMedia />}\n  after={<AfterMedia />}\n  points={[{ label: \"Decision\", content: \"...\" }]}\n  tone=\"blue\"\n/>", props: [{ name: "badge", type: "ReactNode", description: "Scenario or moment label shown in the frame header." }, { name: "title", type: "ReactNode", description: "Primary heading for the comparison frame." }, { name: "before", type: "ReactNode", description: "Route-provided before-state content." }, { name: "after", type: "ReactNode", description: "Route-provided after-state content." }, { name: "beforeLabel / afterLabel", type: "ReactNode", description: "Labels that enable the internal BeforeAfterPanel shells." }, { name: "points", type: "BeforeAfterNarrativePoint[]", description: "Optional narrative points supplied by the route." }, { name: "tone", type: "\"blue\" | \"cyan\" | \"purple\" | \"neutral\"", description: "Tone preset for panel accent styling." }, { name: "connector", type: "ReactNode", description: "Optional custom connector between before and after panels." }, { name: "className slots", type: "string", description: "Optional className hooks for route-owned sizing and media context." }], notes: ["BeforeAfterPanel is rendered internally by this frame.", "Routes own screenshots, redlines, sizing, and project-specific judgment."] }, codeGuidanceZh: { importPath: "components/case-study/BeforeAfterNarrativeFrame.tsx", example: "import { BeforeAfterNarrativeFrame } from \"@/components/case-study/BeforeAfterNarrativeFrame\";\n\n<BeforeAfterNarrativeFrame\n  badge=\"Scenario 1\"\n  title=\"AI Chatbot component\"\n  beforeLabel=\"Before\"\n  afterLabel=\"After\"\n  before={<BeforeMedia />}\n  after={<AfterMedia />}\n  points={[{ label: \"Decision\", content: \"...\" }]}\n  tone=\"blue\"\n/>", props: [{ name: "badge", type: "ReactNode", description: "frame header 內的情境或比較時刻標籤。" }, { name: "title", type: "ReactNode", description: "比較 frame 的主標題。" }, { name: "before", type: "ReactNode", description: "route 提供的 before-state content。" }, { name: "after", type: "ReactNode", description: "route 提供的 after-state content。" }, { name: "beforeLabel / afterLabel", type: "ReactNode", description: "啟用 internal BeforeAfterPanel shell 的 labels。" }, { name: "points", type: "BeforeAfterNarrativePoint[]", description: "route 提供的 optional narrative points。" }, { name: "tone", type: "\"blue\" | \"cyan\" | \"purple\" | \"neutral\"", description: "panel accent styling 的 tone preset。" }, { name: "connector", type: "ReactNode", description: "before / after panel 之間的 optional custom connector。" }, { name: "className slots", type: "string", description: "route-owned sizing 與 media context 的 optional className hooks。" }], notes: ["BeforeAfterPanel 由這個 frame 內部渲染。", "截圖、redline、尺寸與專案判斷由 route 負責。"] }, tokenMappings: [{ token: ".cs-before-after-narrative", role: "Root shell for the guided narrative frame." }, { token: ".cs-before-after-narrative[data-tone]", role: "Tone-specific accent mapping derived from existing case-study tokens." }, { token: "--cs-before-after-narrative-panel-*", role: "Route-overridable panel shell values." }, { token: "--cs-before-after-state-panel-*", role: "Internal BeforeAfterPanel visual shell values." }, { token: "--hm-blue / --hm-purple / --cs-accent", role: "Tone mapping used by the narrative frame and route themes." }], tokenMappingsZh: [{ token: ".cs-before-after-narrative", role: "引導式 narrative frame 的 root shell。" }, { token: ".cs-before-after-narrative[data-tone]", role: "由既有 case-study tokens 推導的 tone-specific accent mapping。" }, { token: "--cs-before-after-narrative-panel-*", role: "route 可覆寫的 panel shell values。" }, { token: "--cs-before-after-state-panel-*", role: "internal BeforeAfterPanel visual shell values。" }, { token: "--hm-blue / --hm-purple / --cs-accent", role: "narrative frame 與 route theme 使用的 tone mapping。" }], accessibility: ["Before / after labels must be readable.", "The comparison should not rely on arrow direction alone.", "Panel content must remain meaningful when stacked.", "Narrative copy should not duplicate inaccessible visual-only content."], accessibilityZh: ["Before / after labels 必須可讀。", "比較關係不能只依賴箭頭方向。", "窄版堆疊時，panel 內容仍需有意義。", "敘事文字不應只是重複不可讀的純視覺資訊。"], tokens: ["--cs-before-after-narrative-panel-*", "--cs-before-after-state-panel-*", "--cs-accent", "--hm-space-md"], referenceCards: [{ label: "Source", value: "components/case-study/BeforeAfterNarrativeFrame.tsx" }, { label: "Live usage routes", value: "Advantech / Crypto Arsenal / Laushu" }, { label: "Internal part", value: "BeforeAfterPanel" }, { label: "Boundary", value: "Route owns media, redlines, sizing, and project-specific judgment" }], referenceCardsZh: [{ label: "Source", value: "components/case-study/BeforeAfterNarrativeFrame.tsx" }, { label: "Live usage routes", value: "Advantech / Crypto Arsenal / Laushu" }, { label: "Internal part", value: "BeforeAfterPanel" }, { label: "Boundary", value: "media、redline、尺寸與專案判斷由 route 負責" }], references: ["components/case-study/BeforeAfterNarrativeFrame.tsx", "components/case-study/BeforeAfterPanel.tsx", "styles/case-study.css"] },
  {
    ...
    stateRowsZh: [
      { state: "Default image", appliesTo: "ZoomableImage trigger", trigger: "圖片渲染在案例頁 media slot 中。", behavior: "trigger 顯示圖片與放大 affordance。", whatChanges: "`.cs-zoomable-image` render button、`next/image` 與 `.cs-zoomable-icon`。", liveUsage: "Laushu media sections；Advantech 與 Crypto Arsenal lightbox wrappers。" },
      { state: "Hover / focus", appliesTo: "ZoomableImage trigger", trigger: "滑鼠 hover 或鍵盤 focus。", behavior: "圖片得到輕微 emphasis 與 focus outline。", whatChanges: "Production CSS 調整 media scale / brightness 與 icon opacity。", liveUsage: "案例頁圖片檢視 affordance。" },
      { state: "Opened lightbox", appliesTo: "Lightbox dialog", trigger: "使用者啟動圖片 trigger。", behavior: "portal dialog 會在頁面上方開啟。", whatChanges: "`.cs-zoomable-lightbox` mount，包含 `role=\"dialog\"`、`aria-modal`、close button 與 scroll lock。", liveUsage: "放大檢視產品截圖。" },
      ...
      { state: "Fullscreen mode", appliesTo: "Lightbox chrome", trigger: "提供 `lightboxMode=\"fullscreen\"`。", behavior: "同一元件套用 fullscreen lightbox styling。", whatChanges: "Root lightbox 取得 `.is-fullscreen`。", liveUsage: "需要檢視大型媒體時的 route-specific 狀態。" },
    ],
    anatomyParts: [
    ...
    anatomyPartsZh: [
      ...
      { part: "Image", description: "使用 route 傳入的 src、alt、width、height 與 sizes 渲染 Next Image。", owner: "ZoomableImage", code: ".cs-zoomable-image-media" },
      { part: "Zoom icon", description: "疊在圖片上的裝飾型放大 affordance。", owner: "ZoomableImage", code: ".cs-zoomable-icon" },
      ...
      { part: "Route media shell", description: "caption、crop 與外層 layout 由 CaseMedia 或 route wrapper 負責。", owner: "Route / CaseMedia", code: "CaseMedia / route CSS" },
    ],
    codeGuidance: {
    ...
    codeGuidanceZh: {
      ...
      props: [
        ...
        { name: "alt", type: "string", description: "有意義的圖片替代文字，也作為 dialog label 來源。" },
        ...
        { name: "lightboxMode", type: "\"default\" | \"fullscreen\"", description: "Optional lightbox chrome mode。" },
        { name: "className / imageClassName", type: "string", description: "route-owned media crop 與 sizing styling hooks。" },
        { name: "sizes", type: "string", description: "Optional responsive image sizes string。" },
      ],
      notes: ["使用 asset 的真實尺寸。", "crop、frame 與 caption 決策留在 CaseMedia 或 route CSS。"],
    },
    tokenMappings: [
    ...
    tokenMappingsZh: [
      { token: ".cs-zoomable-image", role: "互動 trigger root、圖片尺寸、cursor 與 focus target。" },
      { token: ".cs-zoomable-image-media", role: "圖片顯示、responsive width、hover / focus transform。" },
      { token: ".cs-zoomable-icon", role: "裝飾型 zoom affordance overlay。" },
      { token: ".cs-zoomable-lightbox", role: "Fixed overlay、dialog layer、backdrop 與 scroll-lock context。" },
      { token: ".cs-zoomable-lightbox-close", role: "關閉按鈕的位置、surface 與 focus treatment。" },
    ...
    referenceCardsZh: [
      { label: "Source", value: "components/case-study/ZoomableImage.tsx" },
      { label: "Live usage", value: "Laushu media sections；Advantech FeatureImageLightbox；Crypto Arsenal StepLightbox" },
      { label: "Boundary", value: "可放大檢視的案例頁視覺證據，不是 generic gallery" },
      { label: "Related", value: "CaseMedia / route-specific image lightbox wrappers" },
    ],
  ...
  },
  {
    ...
    usageZh: [
      ...
      "內容不需要水平捲動時不要使用。",
      "不要當成 generic carousel indicator 使用。",
    ],
    stateRows: [
    ...
    stateRowsZh: [
      ...
      { state: "No overflow", appliesTo: "Following sibling scroll container", trigger: "沒有找到真正可水平捲動的容器。", behavior: "提示維持隱藏。", whatChanges: "`data-visible` 維持 `false`；production CSS 讓 `.cs-flow-hint` 隱藏。", liveUsage: "board 可以完整容納的桌機版 layout。" },
      ...
      { state: "Decorative hint", appliesTo: "Assistive technology", trigger: "提示被 render。", behavior: "對 screen reader 隱藏。", whatChanges: "Root 保持 `aria-hidden=\"true\"`。", liveUsage: "只作為視覺 scroll affordance。" },
    ],
    anatomyParts: [
    ...
    anatomyPartsZh: [
      ...
      { part: "Label", description: "由 route 提供的在地化視覺 label。", owner: "Route / caller", code: "label" },
    ...
    tokenMappingsZh: [
      ...
      { token: "@theme overrides", role: "Route theme 可調整 overflow board 前後的 spacing 或 color。" },
    ],
    ...
    accessibilityZh: ["提示是裝飾型，目前使用 `aria-hidden=\"true\"`。", "它不能是存取水平 overflow 內容的唯一方式。", "真實 scroll container 仍應依 route 需求保持 touch、pointer 與鍵盤可操作。", "沒有真實水平 overflow 時不要顯示提示。"],
    referenceCards: [
    ...
    referenceCardsZh: [
      { label: "Source", value: "components/case-study/FlowScrollHint.tsx" },
      { label: "Live usage", value: "Advantech AnalysisSection / ProcessSection / ScenarioSection；Laushu 寬版資料表" },
      { label: "Boundary", value: "案例頁 flow overflow affordance，不是 generic carousel control" },
      { label: "Related", value: "CaseFlowFrame / CaseMedia scroll variant / route-specific boards" },
    ],
...
const references: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    description: "Boundary notes clarify which UI pieces are general-purpose components, shared patterns, shared case-study patterns, route-local patterns, or internal anatomy.",
    descriptionZh: "Boundary notes 用來區分哪些 UI 適合放進通用元件目錄，哪些屬於 shared pattern、shared case-study pattern、route-local pattern 或內部組成結構。",
    category: "Reference",
    usage: [
      "Use this reference to separate general-purpose components from shared conventions, case-study patterns, route-specific compositions, and internal implementation parts.",
    ...
    usageZh: [
      "用這份 reference 區分通用元件、shared conventions、case-study patterns、route-specific compositions 與內部實作組成。",
      "shared pattern 可以先文件化重複出現的結構，不必假裝它已經有穩定的獨立 component API。",
