...
const references: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    usage: [
      ...
      "Future candidates not shown in the live catalog: Radio and Alert. Radio may be useful if the Contact form later adds a small set of inquiry-type choices. Alert may be useful if failed submissions need a persistent error summary.",
      "Backlog contracts not shown in this catalog: Select, Checkbox, and EmptyState. They remain source-level candidates only. They should be documented here only after a real portfolio route adopts them.",
    ...
    usageZh: [
      ...
      "未放入 live catalog 的 future candidates：Radio、Alert。若 Contact 表單未來需要少量詢問類型選項，Radio 可能適合；若送出失敗需要持續顯示的錯誤摘要，Alert 可能適合。",
      "未放入本 catalog 的 backlog contracts：Select、Checkbox、EmptyState。它們目前只保留為 source-level candidates；等正式作品集 route 真的採用後，才回到這份文件站。",
