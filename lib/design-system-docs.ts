...
const foundations: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    usageZh: [
      ...
      "關鍵操作不能只有 icon；只有 icon 的按鈕一定要有 accessible label。",
    ...
    referenceCardsZh: [
      { label: "Generic component", value: "目前沒有共用的 `<Icon />` 元件，各元件自行 import 需要的 icon。" },
      { label: "Token policy", value: "沒有 `--hm-icon-*` tokens；icon 直接沿用既有的文字、狀態、間距和動效 tokens。" },
    ],
    references: [
      "package.json",
...
const componentSeeds: ComponentSeed[] = [
  {
    ...
    referenceCardsZh: [
      { label: "Source", value: "components/ui/Button.tsx" },
      { label: "Live usage", value: "首頁 Hero / Contact 表單 / ProjectCard CTA / 案例頁導覽" },
      { label: "CSS classes", value: ".ds-button / .ds-button-primary / .ds-button-secondary / .ds-button-spinner" },
      { label: "Boundary", value: "Button = action / submit；LinkButton = navigation via href" },
    ],
  },
  ...
  },
  {
    ...
    referenceCards: [
      ...
      { label: "Live usage", value: "Public portfolio and case-study shells" },
...
const references: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    descriptionZh: "Boundary notes 用來區分哪些 UI 適合放進通用元件目錄，哪些屬於 shared pattern、shared case-study pattern、route-local pattern、內部組成結構，或 internal documentation shell anatomy。",
    category: "Reference",
    usage: [
    ...
    usageZh: [
      "用這份 reference 區分通用元件、shared conventions、case-study patterns、route-specific compositions、內部實作組成，以及 documentation shell anatomy。",
      ...
      "internal documentation shell anatomy 用於記錄只服務 design-system documentation shell 本身（例如它的 sidebar navigation）的部分，這些不是通用 portfolio component。",
