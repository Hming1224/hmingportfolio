...
const foundations: DesignSystemDoc[] = [
  ...
  },
  {
    ...
    usage: [
      "Use icons to support navigation, actions, status, or external links.",
      "Pair icons with visible labels when meaning is not obvious.",
      "Do not use icons as the only cue for critical actions.",
      "Do not introduce a new icon style unless it matches the existing visual language.",
      "Keep icon color aligned with text color, action importance, or status tone.",
    ],
    usageZh: [
      "Icon 用來輔助導覽、操作、狀態或外部連結。",
      "當意思不是一眼可懂時，icon 需搭配可見文字 label。",
      "關鍵操作不能只依賴 icon 作為唯一線索。",
      "不要新增與現有視覺語言不一致的 icon style。",
      "Icon 顏色需跟文字層級、操作重要性或狀態 tone 對齊。",
    ],
    behavior: [
      { label: "Current source", description: "`lucide-react` is the current package source for UI icons; footer and contact social links use image assets." },
      { label: "Inline SVG", description: "LanguageSwitcher and case-study proposal controls use scoped inline SVG when the shape is local to that component." },
      { label: "No generic API", description: "The portfolio does not currently expose a stable `<Icon />` API." },
      { label: "Interaction owner", description: "Interactive states belong to the button, link, or container that owns the action, not to an icon token." },
    ],
    behaviorZh: [
      { label: "Current source", description: "`lucide-react` 是目前 UI icons 的套件來源；Footer 與 Contact social links 使用 image assets。" },
      { label: "Inline SVG", description: "LanguageSwitcher 與案例頁 proposal controls 會在 shape 屬於該元件時使用 scoped inline SVG。" },
      { label: "No generic API", description: "作品集目前沒有穩定的 `<Icon />` API。" },
      { label: "Interaction owner", description: "互動狀態由擁有操作的 button、link 或 container 承擔，不由 icon token 承擔。" },
    ],
    accessibility: [
      "Decorative icons should be hidden from assistive technology when adjacent text already communicates the meaning.",
      "Meaningful icon-only buttons need an accessible label.",
      "External and social links need clear labels even when the visual icon is decorative.",
      "Status icons should support text and roles such as `status` or `alert`; do not rely on color alone.",
      "Do not claim screen-reader behavior that the implementation does not provide.",
    ],
    accessibilityZh: [
      "如果旁邊已有文字說明，裝飾性 icon 應對 assistive technology 隱藏。",
      "有語意的 icon-only button 需要 accessible label。",
      "外部連結與社群連結即使視覺 icon 是裝飾，也需要清楚 label。",
      "狀態 icon 需要搭配文字與 `status` / `alert` 等 role，不只靠顏色。",
      "不要宣稱 implementation 沒有提供的 screen reader 行為。",
    ],
    tokens: ["--hm-ink / --text-*", "--hm-success / --hm-error / --hm-info", "--hm-space-2xs", "--hm-duration-fast"],
    referenceCards: [
      { label: "Primary package", value: "lucide-react" },
      { label: "Asset usage", value: "Footer and contact social links use `/social/*` image assets." },
      { label: "Generic component", value: "No visible Icon component and no stable `<Icon />` API." },
      { label: "Token policy", value: "No `--hm-icon-*` tokens; icon styling follows existing text, spacing, and status tokens." },
    ],
    referenceCardsZh: [
      { label: "Primary package", value: "lucide-react" },
      { label: "Asset usage", value: "Footer 與 Contact social links 使用 `/social/*` image assets。" },
      { label: "Generic component", value: "沒有 visible Icon component，也沒有穩定 `<Icon />` API。" },
      { label: "Token policy", value: "沒有 `--hm-icon-*` tokens；icon styling 依附既有 text、spacing 與 status tokens。" },
    ],
    references: [
      "package.json",
