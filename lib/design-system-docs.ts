...
const componentSeeds: ComponentSeed[] = [
  ...
  { slug: "case-toc", title: "CaseTOC", titleZh: "案例目錄", category: "Navigation", source: "components/CaseTOC.tsx", demo: "case-toc", states: ["hidden before content", "visible", "active section"], usage: ["Real usage: CaseStudyShell across Advantech, Crypto Arsenal, and Laushu case routes.", "Supports long-form reading, section anchors, active section state, and floating navigation tied to case-study scroll position.", "Protected case-study navigation pattern; do not wrap it into a generic docs TOC."], usageZh: ["真實使用位置：Advantech、Crypto Arsenal、Laushu case routes 的 CaseStudyShell。", "支援長篇案例閱讀、section anchors、active section state，以及綁定 case-study scroll position 的 floating navigation。", "受保護的 case-study navigation pattern；不要包裝成 generic docs TOC。"], tokens: ["--hm-ink", "--hm-muted", "--hm-duration-fast"], accessibility: ["Uses nav semantics with section links and aria-current for the active section."], accessibilityZh: ["使用 nav 語意、章節連結，並用 aria-current 標示 active section。"] },
  ...
  { slug: "case-before-after", title: "CaseBeforeAfter", titleZh: "前後比較", category: "Case Study", source: "components/case-study/CaseBeforeAfter.tsx", demo: "case-before-after", states: ["desktop horizontal", "mobile stacked"], usage: ["Independent two-panel before / after comparison component.", "Current route adoption: no direct usage in Advantech, Crypto Arsenal, or Laushu.", "Boundary: not a BeforeAfterNarrativeFrame replacement or live route example.", "The .cs-before-after-panel selector boundary belongs to CaseBeforeAfter."], usageZh: ["獨立的兩欄 before / after 比較元件。", "目前正式案例頁使用狀態：Advantech、Crypto Arsenal、Laushu 沒有直接使用。", "邊界：不是 BeforeAfterNarrativeFrame 的替代元件，也不作為 live route example 呈現。", "`.cs-before-after-panel` selector 邊界屬於 CaseBeforeAfter。"], tokens: ["--cs-surface", "--cs-line", "--cs-accent", "--hm-space-md"] },
  ...
  { slug: "before-after-panel", title: "BeforeAfterPanel", titleZh: "Before / After 視覺面板", category: "Case Study", source: "components/case-study/BeforeAfterPanel.tsx", demo: "before-after-panel", states: ["blue", "cyan", "purple", "neutral"], usage: ["Internal-only anatomy.", "Used by BeforeAfterNarrativeFrame to render labeled before / after state panels.", "Not directly adopted by routes and not exposed as a standalone production pattern.", "CaseBeforeAfter has its own internal panel function; its .cs-before-after-panel selector is a separate boundary."], usageZh: ["Internal-only anatomy。", "由 BeforeAfterNarrativeFrame 用來呈現 labeled before / after state panels。", "沒有被 route 直接採用，也不是獨立 production pattern。", "CaseBeforeAfter 有自己的內部 panel function；它的 `.cs-before-after-panel` selector 是另一個邊界。"], tokens: ["--cs-before-after-state-panel-*", "--cs-accent", "--cs-surface", "--cs-line"] },
