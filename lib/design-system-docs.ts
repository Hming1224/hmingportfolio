...
const componentSeeds: ComponentSeed[] = [
  ...
  { slug: "language-switcher", title: "LanguageSwitcher", titleZh: "語系切換", category: "General", source: "components/LanguageSwitcher.tsx", demo: "language-switcher", states: ["closed", "open", "selected", "loading"], usage: ["Real usage: global site header.", "Shown inside a navbar crop; only the language switcher is interactive in this docs example.", "Production behavior preserves the current route and hash when switching locale.", "Docs demo uses local state instead of navigation so it cannot change the current page."], usageZh: ["真實使用位置：全站 Navbar。", "文件站以 navbar 截取示意呈現，只有語言切換器可互動。", "正式站切換語系時會保留目前 route 與 hash。", "文件站 demo 使用 local state，不執行導頁，因此不會改變目前頁面。"], tokens: ["--hm-surface", "--hm-ink", "--hm-line", "--hm-radius-md"], accessibility: ["Uses button semantics with aria-expanded for the menu trigger.", "Options use menuitemradio with aria-checked for selected state."], accessibilityZh: ["選單 trigger 使用 button 與 aria-expanded 標示狀態。", "選項使用 menuitemradio 與 aria-checked 表示 selected state。"] },
