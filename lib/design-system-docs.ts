...
const componentSeeds: ComponentSeed[] = [
  ...
  { slug: "before-after-narrative-frame", title: "BeforeAfterNarrativeFrame", titleZh: "敘事型前後比較框架", category: "Case Study", source: "components/case-study/BeforeAfterNarrativeFrame.tsx", demo: "before-after-narrative", states: ["blue", "cyan", "purple", "neutral", "slot-based"], usage: ["Live example directly renders BeforeAfterNarrativeFrame with Advantech SolutionSection Board 1 context.", "Implemented shared slot-based narrative layout frame for single-comparison before / after stories.", "Owns the CaseCard shell, header / badge / title anatomy, optional intro, points, panel layout, connector, and data-tone hook.", "BeforeAfterPanel is rendered inside this frame as an internal visual shell; it is not a standalone route-level pattern.", "Does not own media wrappers, lightbox behavior, redlines, image sizing, crop, figure semantics, or route-specific storytelling geometry.", "Adopted by Crypto IterationSection, Laushu iteration board, and Advantech Board 1 pilot; Advantech Board 2 / 3 remain route-local."], usageZh: ["範例直接 render BeforeAfterNarrativeFrame，語境對齊 Advantech SolutionSection Board 1。", "已實作的 slot-based 敘事型 before / after 共用 layout frame，適合單一比較故事。", "負責 CaseCard 外框、header / badge / title、optional intro、points、panel layout、connector 與 data-tone hook。", "BeforeAfterPanel 在這個 frame 內作為 internal visual shell 被 render；它不是 standalone route-level pattern。", "不負責 media wrapper、lightbox、redline、圖片尺寸 / 裁切、figure 語意或 route-specific storytelling geometry。", "已用於 Crypto IterationSection、Laushu iteration board、Advantech Board 1 pilot；Advantech Board 2 / 3 仍保留 route-local。"], tokens: ["--cs-accent", "--cs-surface", "--cs-line", "--hm-space-md", "--cs-before-after-state-panel-*"] },
