...
const decisionLog = [
  ...
  "StatusBadge 這類還沒有真實使用場景的元件，一律暫緩建立。",
  "未上線的案子用 disabled 底色呈現，不做假連結騙點擊。",
  "文件目錄只列 production 實際使用的元件。文件站上線後，我逐項稽核 30 個項目，確認每一項都能對上實際頁面。",
...
export default async function GovernanceSection() {
  ...
  return (
    <CaseSection id="cs-sec-governance" kicker={t("GOVERNANCE")} title={t("Governance 與 AI 協作：讓流程可管理、可驗證")} surface>
      ...
      <p className="cs-section-lead">{t("除了元件和設計規則，我也替 AI 協作訂出邊界。最後整理成兩層文件，再加上一份決策紀錄：")}</p>
      <CaseGrid variant="two" className="ds-case-card-grid">
        ...
        <CaseCard>
          ...
          <p>{t("每個 AI-assisted implementation 任務都會分段授權，清楚寫出這一步能做什麼、不能做什麼：audit 只看不改；implementation 不負責 commit；commit 只提交指定檔案；驗證通過後才能 push。這讓我能在每個階段守住修改範圍。")}</p>
        </CaseCard>
      </CaseGrid>
      ...
      <TermNotes
        ...
        items={[
          ...
          { term: t("Feature branch"), description: t("Feature branch 是先把改動放在獨立分支驗證，避免直接影響正式站。") },
