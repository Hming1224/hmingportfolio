...
export default async function RoleSection() {
  ...
  return (
    <section id="cs-sec-role" className="cs-section-surface">
      ...
      <div className="cs-step-flow" role="list" aria-label={t("單一 feature 的設計流程")}>
        {flowSteps.map((step, i) => (
          <div className="cs-step-flow-item" role="listitem" key={step}>
            <CaseCard as="div">
              <span className="cs-step-flow-index">{String(i + 1).padStart(2, "0")}</span>
              <span className="cs-step-flow-title">{t(step)}</span>
            </CaseCard>
            {i < flowSteps.length - 1 && (
              <div className="cs-step-flow-arrow" aria-hidden="true">
