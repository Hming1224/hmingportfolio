...
- Batch 7C-4 update: `CaseHero` stays visible because the shared `CaseHero` source is adopted by Advantech, Crypto Arsenal, and Laushu hero sections. The docs example now renders the real `CaseHero` component with Crypto Arsenal route context instead of a docs-local hero reconstruction.
