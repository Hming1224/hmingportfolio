...
- Motion Foundation consolidation: Motion now follows the same Header, visual samples, When to use, Design Tokens, and Reference structure as the other Foundations pages. Duration and easing remain system-governed visual references, while the production-backed Logo and language-loading animations appear as a third visual sample group. Their timelines remain embedded in Lottie JSON rather than inheriting Motion tokens. Duration and easing group wrappers are layout-only with no nested surface styling. Token Reference remains under Reference; no standalone production-animation page, generic Animation component, or runtime token was introduced.
