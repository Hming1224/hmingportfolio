...
- Motion inventory Batch 1 update: Foundations / Motion now documents Brand motion, Interaction motion, and Reduced motion. Feedback documents Loading state as a reference, not a new Loading component. Skeleton, Toast, and Modal include concise motion notes for shimmer, enter feedback, and minimal modal motion. Component Boundaries now records route-local narrative motion for Home hero decorations, Design System scroll gate, About Genie / Avatar / educator masonry motion, and case-study narrative visuals. AvatarProfile reduced-motion remains a P1 follow-up; WorkCategoryTabs reduced-motion remains a P2 follow-up. This batch did not change production animation, production routes, or `styles/tokens.css`.
- Motion visual examples use the production-backed Logo and language-loading animations inside the first Brand motion behavior row. They are not repeated as separate example cards or role cards above the token reference.
- Motion visual preview enhancement: Foundations / Motion now adds docs-only visual preview cards plus duration and easing reference samples for existing motion tokens. The update stays visual-first, respects reduced-motion in the docs preview, and does not add a Motion component, runtime token, production animation, or `styles/tokens.css` change.
- Motion examples correction: Foundations / Motion examples now foreground production-backed Logo animation (`components/brandLogoAnim.json`, used by `AnimatedLogo`) and Language loading animation (`/animations/language-loading.json`, used by `LanguageSwitcher`). Brand / Interaction / Reduced motion remain supporting guidance, not standalone visible catalog components. AvatarProfile reduced-motion remains a separate P1 follow-up.
