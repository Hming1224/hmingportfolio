...
- Batch Foundations Icons update: `Icons / Iconography` was added as a Foundations reference, not as a visible component catalog item. Current evidence shows `lucide-react`, inline SVG, and social image assets in production usage, but no stable generic `<Icon />` API.
